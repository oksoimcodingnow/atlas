...
(function () {
  ...
  // This script's own absolute URL (e.g. https://…/atlas/lib/atlas-fx.js). Captured
  // now because document.currentScript is only valid during initial sync execution.
  // Used to locate sibling files (sw.js at the repo root) no matter how deep the page is.
  ...
  function startBackground() {
    ...
    // The 3D background is GPU work. On a machine without GPU acceleration
    // (software/SwiftShader rendering — common on 32-bit or old-driver Chrome)
    // it runs on the CPU and lags badly. Detect that and lighten, or bail to a
    // static gradient. A user override (atlas_fx=lite|off) always wins.
    function detectTier() {
      try {
        const ov = localStorage.getItem('atlas_fx');
        if (ov === 'off') return 'off';
        if (ov === 'lite') return 'lite';
      } catch (e) {}
      try {
        const gl = document.createElement('canvas').getContext('webgl') ||
                   document.createElement('canvas').getContext('experimental-webgl');
        if (!gl) return 'off'; // no WebGL at all → static bg
        const dbg = gl.getExtension('WEBGL_debug_renderer_info');
        const r = dbg ? String(gl.getParameter(dbg.UNMASKED_RENDERER_WEBGL)).toLowerCase() : '';
        // software renderers → lite (or off if truly basic)
        if (/swiftshader|software|llvmpipe|microsoft basic|mesa offscreen/.test(r)) return 'lite';
      } catch (e) {}
      // very low core count is another weak-machine signal
      if ((navigator.hardwareConcurrency || 8) <= 2) return 'lite';
      return 'full';
    }
    let tier = detectTier();
    if (tier === 'off') return; // leave the CSS gradient bg only — instant, no 3D

    const renderer = new THREE.WebGLRenderer({ canvas, alpha: true, antialias: tier === 'full' });
    renderer.setPixelRatio(Math.min(devicePixelRatio, tier === 'full' ? 2 : 1));
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(60, innerWidth / innerHeight, 0.1, 1000);
    camera.position.z = 50;

    const isMobile = matchMedia('(max-width: 700px)').matches;
    const N = tier === 'lite' ? 45 : (isMobile ? 80 : 140);
    ...
    // FPS watchdog: even if detection said "full", a struggling machine gets
    // caught here and downgraded once. Also caps lite tier to ~30fps.
    let lastT = performance.now(), frames = 0, fpsT = lastT, downgraded = (tier === 'lite');
    const minInterval = tier === 'lite' ? 1000 / 30 : 0;

    function animate(now) {
      requestAnimationFrame(animate);
      if (!visible) return;
      now = now || performance.now();
      if (minInterval && now - lastT < minInterval) return; // frame cap
      lastT = now;

      // sample FPS over the first ~2s; if it's chugging, downgrade once
      if (!downgraded) {
        frames++;
        if (now - fpsT >= 2000) {
          ...
          if (fps < 30) {
            ...
            for (let i = N - 1; i >= 45; i--) { /* hide extra points via draw range */ }
            pg.setDrawRange(0, 45);
          }
          frames = 0; fpsT = now;
        }
      }
  ...
  function registerSW() {
    ...
    // sw.js lives at the repo root; THIS script lives in /lib. Resolve relative to
    // this script's URL (not the page) so a page in /fineng or /learn doesn't try to
    // register a non-existent fineng/sw.js and fail silently. Falls back to 'sw.js'
    // for the root pages if currentScript was unavailable.
    let swUrl = 'sw.js';
    try { if (SELF_SRC) swUrl = new URL('../sw.js', SELF_SRC).href; } catch (e) { /* keep fallback */ }
    navigator.serviceWorker.register(swUrl)
      .catch((e) => console.warn('atlas-fx: service worker registration failed', e));
  }
