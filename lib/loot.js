...
export class LootEngine {
  ...
  /**
   * Award plain Spirit without rolling loot (e.g. a correct quiz answer or a
   * finished task). Keeps the engine the single owner of the Spirit balance.
   * @param {number} n amount to add (negative is ignored).
   * @returns {number} the new total.
   */
  award(n) {
    if (typeof n === 'number' && n > 0) { this.state.spirit = (this.state.spirit || 0) + n; this._save(); }
    return this.spirit;
  }
